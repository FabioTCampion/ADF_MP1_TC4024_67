    ...
    <>
      ...
      <section className="weights-table" aria-label="Histórico de pesos capturados">
        ...
        {!loading && filteredCaptures.map((capture) => {
          ...
          return (
            <div className={`weights-row${corrected ? " weights-row--corrected" : ""}`} key={capture.id}>
              ...
              <div className="weights-actions">
                {canEdit ? (
                  <button type="button" onClick={() => openEditor(capture)}>{corrected ? "Revisar" : "Corrigir"}</button>
                ) : (
                  <span>—</span>
                )}
              </div>
